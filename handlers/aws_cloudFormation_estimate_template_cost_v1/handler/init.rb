...
class AwsCloudFormationEstimateTemplateCostV1
  ...
  def execute()
    resp = @awsClient.estimate_template_cost(
      template_url: @parameters['template_url'],
      parameters: @template_parameters
    )

    puts "Returning results - - URL: #{resp.url}"  if @enable_debug_logging
    <<-RESULTS
    <results>
      <result name="URL">#{resp.url}</result>
    </results>
    RESULTS
  end
